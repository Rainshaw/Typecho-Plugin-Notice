...
class Title extends Typecho_Widget_Helper_Layout
{
    ...
    {
        if ($isOpen) {
            $this->addItem(new CustomLabel('<div class="mdui-panel-item mdui-panel-item-open">'));
        } else {
            $this->addItem(new CustomLabel('<div class="mdui-panel-item">'));
        }


        $this->addItem(new CustomLabel('<div class="mdui-panel-item-header">' . $titleName . '<small class="mdui-panel-item-sub-header">' . $subtitleName . '</small></div>'));
